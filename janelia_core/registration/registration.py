...
def calc_dataset_ref_image(dataset: DataSet, img_field: str, ref_inds: np.ndarray,
    ...
    ref_stats = std_through_time(images=ref_image_files, preprocess_func=med_filter_image, sc=sc)
    t1 = time()
...
def register_dataset_images(dataset: DataSet, img_field: str, ref_image: np.ndarray,
    ...
    from janelia_core.dataprocessing.utils import get_image_data # Import here to avoid circular imports
