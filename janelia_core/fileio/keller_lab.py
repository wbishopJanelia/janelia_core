...
def read_exp(image_folder: pathlib.Path, meta_data_folder: pathlib.Path,
             image_ext: str='.weightFused.TimeRegistration.klb', meta_data_file: str='ch0.xml',
    ...
    This function assumes all the imaging files are stored the one img_folder in the form:
        img_folder/TM[smp_number]/*.image_ext

    It also assumes the file with the metadata is stored at:
        meta_data_folder/meta_data_file

    and the time point data is stored at:
        meta_data_folder/time_stamps_file
    ...
    Args:
        ...
        meta_data_folder: The folder containing metadata (see above) as a pathlib.Path object

        image_ext: String to use when searching for files with the extension for image files.

        meta_data_file: Name of file containing metadata.
    ...
    metadata = read_metadata(meta_data_folder / meta_data_file)
    time_stamps = read_time_stamps(meta_data_folder / time_stamps_file)
...
def read_images(image_folder: pathlib.Path, image_ext: str='.weightFused.TimeRegistration.klb', verbose=True) -> list:
    ...
    # Indices in file names indicating which sample they correspond to
    start_smp_n_ind = 8
    end_smp_n_ind = 14
    ...
    smp_inds = np.asarray([int(f.name[start_smp_n_ind:end_smp_n_ind]) for f in files_as_paths])
