...
def img_files_to_dask_array(f_names: list) -> dask.array.core.Array:
    ...
    return stack([from_delayed(delayed(read_img_file, pure=True)(f), shape, dtype) for f in f_names])


def read_img_file(f_name: pathlib.Path) -> numpy.ndarray:
